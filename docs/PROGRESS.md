...
- 單位已具備攻擊、待機與可行動狀態，移動與攻擊可行性會由目前回合狀態推導。
- 玩家可用暫時鍵盤操作讓選取單位待機；待機後仍可選取查看狀態，但不能再操作該單位。
- 選取玩家單位後會先停在查看狀態，可用鍵盤切換移動、攻擊或待機；移動與攻擊模式會分別決定後續點擊行為。
...
下一個實作步驟：將行動模式移出單位資料 6D。

## 進度紀錄

### 2026-06-30T10:00:39+0800

- 調整第三個可玩目標的後續順序：在顯示可攻擊範圍前，先加入 6D「將行動模式移出單位資料」。
- 原本的顯示可攻擊範圍順延為 6E，簡單行動選單 UI 順延為 6F。
- 下一步進入 6D：將目前行動模式視為玩家操作狀態，避免移動或攻擊模式下點擊友軍時切換選取單位。

### 2026-06-26T12:12:47+0800

- 完成 6C：新增 `UnitActionMode`，選取單位後先停在查看狀態，並可用鍵盤切換移動、普通攻擊與待機。
- 玩家點擊行為現在依目前模式分流；移動模式不攻擊，攻擊模式不移動。
- 移動範圍 UI 改為依 `CanMoveThisTurn` 判斷，攻擊後若仍有移動力仍可顯示可移動範圍。
...
### 2026-06-25T15:49:11+0800

- 完成 6A 與 6B：新增單位待機狀態、可移動/可攻擊推導屬性，以及暫時鍵盤待機操作。
- 待機後會清除選取單位；同一回合仍可再次選取查看狀態，但不能繼續操作。
- 使用 `dotnet format SRPG_practice.sln --verify-no-changes` 與 `dotnet build SRPG_practice.sln` 驗證通過；build succeeded，0 warnings，0 errors。
- 下一步進入 6C：選取單位後用模式決定點擊行為。

### 2026-06-25T09:23:44+0800

- 調整後續學習任務卡格式：只保留目標、完成條件與不做的事情三段。
- 完成條件改為描述可驗收結果，不列實作細節，讓使用者先自行思考設計。
- 任務卡會使用簡潔項目符號，並可加入少量 emoji 提升可讀性。

### 2026-06-24T16:39:56+0800

- 調整第三個可玩目標的後續順序：先完成單位行動狀態、待機、行動模式、攻擊範圍 UI 與簡單行動選單，再進入法師 MP 與技能選擇。
- 在 `docs/GOALS.md` 新增 6A-6E 的切分，讓後續每一步都能維持小範圍與可 review。
- 下一步先做 6A：新增單位行動狀態概念，暫不改現有 gameplay。
